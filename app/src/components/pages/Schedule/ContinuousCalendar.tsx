...
export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({
        ...
        {week.map(({ month, day }, dayIndex) => {
          ...
          return (
            <div
              ...
              className={`relative z-10 m-[-0.5px] group aspect-square w-full grow cursor-pointer rounded-xl font-medium transition duration-50 ease-in-out hover:z-20 sm:-m-px sm:size-20 sm:rounded-2xl lg:size-36 lg:rounded-3xl 2xl:size-40 border border-white hover:border-pink ${isToday ? "border-2 border-pink" : ""}`}
            >
              <span
                className={`absolute left-1 top-1 flex size-5 items-center justify-center rounded-full text-xs sm:size-6 sm:text-sm lg:left-2 lg:top-2 lg:size-8 lg:text-base ${
                  isToday ? "bg-pink text-black font-semibold" : "text-pink"
                } ${month < 0 ? "text-pink" : "text-black"}`}
              >
                {day}
              </span>
              {isNewMonth && (
              ...
              >
                <svg
                  className="size-8 scale-90 text-pink transition-all hover:scale-100 group-focus:scale-100"
                  aria-hidden="true"
  ...
  return (
    <div className="w-full border-2 border-black no-scrollbar calendar-container max-h-full overflow-y-scroll rounded-t-2xl bg-white pb-10 text-slate-800 shadow-xl">
      <div className="sticky -top-px z-50 w-full rounded-t-2xl bg-white px-5 pt-7 sm:px-8 sm:pt-8">
        <div className="mb-4 flex w-full flex-wrap items-center justify-between gap-6">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            ...
            <button
              type="button"
              className="whitespace-nowrap rounded-lg bg-pink text-black px-3 py-1.5 text-center text-sm font-medium focus:outline-none focus:ring-4 focus:ring-pink/50 sm:rounded-xl lg:px-5 lg:py-2.5 cursor-pointer"
            >
              + Add Event
            </button>
          </div>
