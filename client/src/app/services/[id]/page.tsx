...
const ServiceDetailsPage = () => {
  ...
  const requiredService = useMemo(() => {
    const res = services.find((s) => s.id === id);
    // console.log(`services : ${services}`)
    console.log("res : ", res);
    if (res) {
      const string_1 = `${dayjs().format("YYYY-MM-DD")} ${res.start_time}`;
      const string_2 = `${dayjs().format("YYYY-MM-DD")} ${res.end_time}`;
  ...
  return (
    <main className="bg-gray-50 min-h-screen pt-[130px] pb-10">
      ...
      <article className=" shadow-2xl overflow-hidden transform transition duration-500 hover:shadow-xl">
          ...
          <div className="lg:col-span-2">
            ...
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-gray-200">
              ...
              <div className="bg-indigo-100  text-sm font-medium px-3 py-1 rounded-full">
                ...
                <span className="text-indigo-800">{requiredService.price}</span>
              </div>
            </header>
            ...
            <section className="mb-8 p-6 bg-gray-50 rounded-lg shadow-inner">
              ...
              <ul className="list-none space-y-3">
                {requiredService.details.map((detail) => (
